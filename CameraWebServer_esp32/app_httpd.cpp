...
static esp_err_t pwm_handler(httpd_req_t *req){
  ...
  // Reuse the existing parse_get helper
  if (parse_get(req, &buf) == ESP_OK) {
    if (httpd_query_key_value(buf, "L", l_str, sizeof(l_str)) == ESP_OK) {
      L = atoi(l_str);
    }
  ...
  // Call into your .ino to actually move motors
  PWM_control(L, R);
